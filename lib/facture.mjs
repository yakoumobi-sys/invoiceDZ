...
/* ---------- Retard (échéance dépassée, solde restant dû) ---------- */
const NON_EN_COURS = ["PAYE", "ACCEPTE", "REFUSE", "LIVRE"];

export function isOverdue(doc, todayISO) {
  if (!doc.dateEcheance || doc.type === "BON_LIVRAISON") return false;
  if (NON_EN_COURS.includes(doc.statut)) return false;
  const today = todayISO || new Date().toISOString().slice(0, 10);
  if (doc.dateEcheance >= today) return false;
  return calcTotaux(doc).solde > 0.009;
}
