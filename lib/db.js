...
/* ---------- profil entreprise ---------- */
const EMPTY_ENT = { nom: "", adresse: "", ville: "", telephone: "", email: "", nif: "", rc: "", ai: "", idFiscal: "", code: "", secteur: "", effectif: "" };

export async function getProfil(user) {
  const local = LS.get("idz.profil", null);
  const meta = user?.user_metadata || {};
  if (meta.entreprise || meta.logo || meta.signature || meta.cachet) {
    return {
      entreprise: { ...EMPTY_ENT, ...(meta.entreprise || {}) },
      logo: meta.logo || null,
      signature: meta.signature || null,
      cachet: meta.cachet || null,
      prefs: meta.prefs || {},
    };
  }
  if (local) return {
    entreprise: { ...EMPTY_ENT, ...(local.entreprise || {}) },
    logo: local.logo || null,
    signature: local.signature || null,
    cachet: local.cachet || null,
    prefs: local.prefs || {},
  };
  return { entreprise: { ...EMPTY_ENT }, logo: null, signature: null, cachet: null, prefs: {} };
}
  ...
  if (user) {
    try {
      await supabase.auth.updateUser({
        data: { entreprise: profil.entreprise, logo: profil.logo, signature: profil.signature, cachet: profil.cachet, prefs: profil.prefs },
      });
    } catch {}
  }
  return profil;
