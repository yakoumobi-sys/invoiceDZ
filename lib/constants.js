...
/* profil : { entreprise, logo, signature, cachet, prefs } — voir lib/db.js getProfil().
   La signature/le cachet ne sont ajoutés au document que si l'utilisateur a coché
   « afficher automatiquement » dans Compte (prefs.afficherSignature). */
export const newDoc = (type, profil = {}) => {
  const prefs = profil.prefs || {};
  const inclureSignature = prefs.afficherSignature === true;
  return {
    type,
    numero: "",
    statut: "BROUILLON",
    ...
    paiements: [],
    entreprise: profil.entreprise || { nom: "", adresse: "", ville: "", telephone: "", email: "", nif: "", rc: "", ai: "", idFiscal: "", code: "" },
    logo: profil.logo || null,
    signature: inclureSignature ? (profil.signature || null) : null,
    cachet: inclureSignature ? (profil.cachet || null) : null,
  };
};
